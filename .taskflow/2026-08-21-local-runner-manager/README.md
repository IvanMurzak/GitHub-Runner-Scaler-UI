...
| D11 | v1 distribution channels are a hosted install script (`curl \| sh`, `irm \| iex`), an npm wrapper package, a Homebrew tap, a Scoop bucket, `cargo install`, and the GitHub Releases archives all of them consume. | Locked 2026-08-21 | Owner decision 2026-08-21. `winget` is excluded from the product entirely: npm covers Windows, and `microsoft/winget-pkgs` moderation would add an external release blocker. The install script is the universal fallback for hosts with no package manager, notably Linux. |
