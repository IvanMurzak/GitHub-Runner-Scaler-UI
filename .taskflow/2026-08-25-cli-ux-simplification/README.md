...
| D4 | The full `Administration: Read and write` disclosure prints on **neither** `auth login` nor `repo add`. `auth login` prints no permission text at all; the grant lives in `README.md` and in the new `auth status --permissions`, which needs no credential and issues no request. `repo add` on a monitor-only policy still prints the three consequence sentences unprompted. | **SUPERSEDED 2026-08-27** by the owner exercising gate **G4**; see the progress log | **D21** (disclosure clause) | The obligation was that *this tool*, and not only GitHub's consent screen, states what the grant permits, and that the statement is reachable. Twenty-five identical lines above the device code met neither: they were the same on every run and for every user, so a returning operator scrolled past them to reach the code. Pulling them out of the busiest screen in the product and putting them behind a command that answers on request keeps the statement and drops the habituation. |
