#!/usr/bin/env bash
set -euo pipefail

die() {
  echo "managed WSL OCI acceptance: $*" >&2
  exit 1
}

[[ "$(uname -s)" == Linux ]] || die "this harness requires Linux"
grep -qi microsoft /proc/sys/kernel/osrelease || die "this harness requires WSL2"
[[ "$(id -u)" -eq 0 ]] || die "run this harness as WSL root to provision the loop mount"

managed_user="${RUNNER_MANAGER_WSL_USER:-}"
[[ -n "$managed_user" ]] || die "RUNNER_MANAGER_WSL_USER must name the managed rootless user"
managed_uid="$(id -u "$managed_user")"
managed_gid="$(id -g "$managed_user")"
managed_home="$(getent passwd "$managed_user" | cut -d: -f6)"
[[ "$managed_uid" -ne 0 ]] || die "Podman must execute rootless"

source_root="$(pwd -P)"
[[ -f "$source_root/Cargo.toml" ]] || die "run from the repository root"
: "${RUNNER_MANAGER_OCI_ACCEPTANCE_IMAGE:?a pinned OCI image digest is required}"
[[ "$RUNNER_MANAGER_OCI_ACCEPTANCE_IMAGE" == *@sha256:* ]] ||
  die "RUNNER_MANAGER_OCI_ACCEPTANCE_IMAGE is not digest pinned"

fixture="$managed_home/.local/share/runner-manager-wsl-oci-acceptance"
image_file="/var/lib/runner-manager-wsl-oci-acceptance.ext4"
mount_point="$fixture/store"
graph_root="$mount_point/graph"
run_root="$fixture/run"
tmp_root="$fixture/tmp"
cargo_target="$fixture/cargo-target"
storage_conf="$fixture/storage.conf"
runtime_helper="/usr/local/libexec/runner-manager-oci-acceptance"
mounted=false
helper_created=false

as_managed() {
  runuser -u "$managed_user" -- env \
    HOME="$managed_home" USER="$managed_user" LOGNAME="$managed_user" \
    XDG_RUNTIME_DIR="/run/user/$managed_uid" \
    DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$managed_uid/bus" \
    CONTAINERS_STORAGE_CONF="$storage_conf" \
    "$@"
}

cleanup() {
  status=$?
  set +e
  if [[ -x "$runtime_helper" ]]; then
    as_managed "$runtime_helper" rm --all --force >/dev/null 2>&1
    as_managed "$runtime_helper" system reset --force >/dev/null 2>&1
  fi
  if $mounted; then
    umount "$mount_point"
  fi
  rm -rf "$fixture"
  rm -f "$image_file"
  if $helper_created; then
    rm -f "$runtime_helper"
  fi
  exit "$status"
}
trap cleanup EXIT

rm -rf "$fixture"
rm -f "$image_file"
[[ ! -e "$runtime_helper" ]] || die "$runtime_helper already exists"
mkdir -p "$mount_point" "$run_root" "$tmp_root" "$cargo_target"
chown -R "$managed_uid:$managed_gid" "$fixture"
truncate -s 1024M "$image_file"
mkfs.ext4 -q -F -m 0 "$image_file"
mount -o loop,nodev,nosuid "$image_file" "$mount_point"
mounted=true
chown "$managed_uid:$managed_gid" "$mount_point"
chmod 0700 "$mount_point" "$run_root" "$tmp_root" "$cargo_target"
install -d -m 0700 -o "$managed_uid" -g "$managed_gid" "$graph_root"
install -d -m 0755 -o root -g root /usr/local/libexec

cat > "$storage_conf" <<EOF
[storage]
driver = "overlay"
runroot = "$run_root"
graphroot = "$graph_root"
rootless_storage_path = "$graph_root"

[storage.options.overlay]
mount_program = "/usr/bin/fuse-overlayfs"
mountopt = "nodev"
EOF

cat > "$runtime_helper" <<'HELPER'
#!/usr/bin/env bash
set -euo pipefail

storage_conf='__STORAGE_CONF__'
graph_root='__GRAPH_ROOT__'
export CONTAINERS_STORAGE_CONF="$storage_conf"

verify_bounded_store() {
  [[ "$(findmnt -T "$graph_root" -no FSTYPE)" == "ext4" ]] || exit 78
  source="$(findmnt -T "$graph_root" -no SOURCE)"
  [[ "$source" == /dev/loop* ]] || exit 78
  read -r block_size block_count < <(stat -f -c '%S %b' "$graph_root")
  capacity=$((block_size * block_count))
  ((capacity <= 1024 * 1024 * 1024)) || exit 78
}

requested=""
command_name=""
args=()
while (($#)); do
  if [[ "$1" == "--storage-opt" ]]; then
    (($# >= 2)) || exit 64
    [[ "$2" == size=* ]] || exit 64
    requested="${2#size=}"
    shift 2
    continue
  fi
  [[ -n "$command_name" || "$1" == -* ]] || command_name="$1"
  args+=("$1")
  shift
done

if [[ -n "$requested" && "$requested" != "1024m" ]]; then
  echo "bounded store has no slot for requested size $requested" >&2
  exit 69
fi

if [[ -n "$requested" ]]; then
  verify_bounded_store
fi

if [[ -n "$requested" && "$command_name" == "info" ]]; then
  raw="$(podman "${args[@]}")"
  BOUNDED_INFO="$raw" python3 - <<'PY'
import json
import os

info = json.loads(os.environ["BOUNDED_INFO"])
info["runnerManagerStorage"] = {
    "schema": 1,
    "mode": "exclusive-filesystem-pool",
    "requestedMiB": 1024,
    "hardCap": True,
}
print(json.dumps(info, separators=(",", ":")))
PY
  exit 0
fi

exec podman "${args[@]}"
HELPER
helper_created=true
sed -i "s|__STORAGE_CONF__|$storage_conf|g; s|__GRAPH_ROOT__|$graph_root|g" "$runtime_helper"
chown root:root "$storage_conf" "$runtime_helper"
chmod 0644 "$storage_conf"
chmod 0755 "$runtime_helper"

mount_evidence="$(findmnt -T "$mount_point" -no TARGET,SOURCE,FSTYPE,OPTIONS)"
echo "$mount_evidence"
grep -Eq '^/home/.* /dev/loop[0-9]+ ext4 .*(nodev|nosuid)' <<<"$mount_evidence" ||
  die "bounded store is not the disposable ext4 loop mount"
for path in "$mount_point" "$graph_root" "$run_root" "$tmp_root" "$runtime_helper"; do
  [[ "$path" == /* && "$path" != /mnt/* ]] || die "runtime state escaped to DrvFS: $path"
done

as_managed "$runtime_helper" --storage-opt size=1024m info --format=json > "$fixture/probe.json"
python3 - "$fixture/probe.json" "$graph_root" <<'PY'
import json
import pathlib
import sys

info = json.loads(pathlib.Path(sys.argv[1]).read_text())
assert info["host"]["security"]["rootless"] is True
assert info["host"]["cgroupVersion"] == "v2"
assert {"cpu", "memory", "pids"} <= set(info["host"]["cgroupControllers"])
assert info["store"]["graphRoot"] == sys.argv[2]
assert info["store"]["graphStatus"]["Backing Filesystem"] == "extfs"
assert info["runnerManagerStorage"] == {
    "schema": 1,
    "mode": "exclusive-filesystem-pool",
    "requestedMiB": 1024,
    "hardCap": True,
}
PY

cd "$source_root"
cargo_bin="$managed_home/.cargo/bin/cargo"
[[ -x "$cargo_bin" ]] || cargo_bin="$(command -v cargo)"
as_managed env \
  TMPDIR="$tmp_root" \
  CARGO_TARGET_DIR="$cargo_target" \
  RUNNER_MANAGER_OCI_RUNTIME="$runtime_helper" \
  RUNNER_MANAGER_OCI_ACCEPTANCE_IMAGE="$RUNNER_MANAGER_OCI_ACCEPTANCE_IMAGE" \
  "$cargo_bin" test -p runner-manager-agent \
    'oci::tests::live_rootless_managed_wsl_bounded_acceptance' -- \
    --ignored --exact --nocapture

remaining="$(as_managed "$runtime_helper" ps --all --quiet)"
[[ -z "$remaining" ]] || die "acceptance containers survived cleanup: $remaining"
if grep -r -a -q 'rm-jit-sentinel-' "$graph_root"; then
  die "a synthetic JIT sentinel survived in bounded runtime storage"
fi
echo "managed WSL bounded-store acceptance passed"
